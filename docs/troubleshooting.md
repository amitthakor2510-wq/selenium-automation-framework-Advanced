...
| `Could not create a session` with `browser=safari` | Remote Automation was never enabled on this machine, or you're not on macOS at all | Run `safaridriver --enable` once (needs `sudo` locally), and confirm you're on a real Mac — there's no Linux/Windows Safari build for `WebDriverManager` to fall back to |
| A `-Dheadless=true` Safari run still opens a visible window | Expected — Safari has no headless mode | `DriverFactory` logs a warning and launches a normal windowed session; drop `-Dheadless` from Safari runs, or ignore the warning |
| Second concurrent Safari session fails outright | Only one `SafariDriver` session may exist on a machine at a time (WebKit/Apple limitation) | Use the dedicated `testng-suites/<site>-safari-<suite>.xml` files (`parallel="none"`) instead of the plain ones — never point a `thread-count>1` suite at `browser=safari` |
| `UploadDownloadTest`-style download assertion fails only under Safari | Safari has no per-session download-directory isolation — files land in the real `~/Downloads`, not `DriverFactory.getDownloadPath()`'s per-thread folder | Not portable to Safari as-is; skip/adjust that assertion for the Safari suite, or check `~/Downloads` directly |
| `docker compose run --rm tests` is clearly running (dashboard shows an active session) but `http://localhost:7900` noVNC shows nothing | `HEADLESS=true` was being forwarded into the **remote** ChromeOptions/FirefoxOptions/EdgeOptions too, so the Grid node's browser launched with `--headless=new` — nothing was ever drawn to the node's internal Xvfb display for noVNC to stream, so the viewer wasn't broken, it just had a genuinely blank screen to show | Already fixed in `DriverFactory`: `headless` is now ignored for Grid (remote) sessions — a normal windowed browser always launches inside the node's Xvfb regardless of `HEADLESS`. Just open `:7900` (chrome) / `:7901` (firefox) / `:7902` (edge), password `secret` |
| Extent report is missing screenshots/tests from a suite you know ran | `ExtentManager.setActiveSuiteName()` was dead code — never called from anywhere — so every suite type (smoke/regression/accessibility/visual) for a given site+browser collapsed onto the same `.../suite/index.html` file; running two suite types back-to-back in one `target/` without `mvn clean` silently overwrote the first suite's report, screenshots included | Fixed — now called from `BaseTest.setUp()`/`MobileBaseTest.setUp()`. Update to a version with this fix, or run `mvn clean` between different suite types against the same site+browser as a workaround on an older build |
| ReportPortal launch shows pass/fail but no screenshots or video | The auto-registered `ReportPortalTestNGListener` agent only reports TestNG's pass/fail/skip lifecycle — it never captured files; nothing in the project called `ReportPortal.emitLog()` with an attachment | Fixed — `TestListener` now emits the same screenshot/video evidence to ReportPortal (via `ReportPortalMessage`) that it already sent to Allure/Extent, gated the same way (no-ops cleanly when `reportportal.enable=false`) |
| `KeywordExecutionException: Unhandled keyword: SOLVE_TEXT_CAPTCHA` (or `SOLVE_MATH_CAPTCHA`/`SOLVE_CAPTCHA_WITH_AI`) | `CaptchaSolver` existed as a standalone class but `KeywordEngine.execute()` had no `case` for any of the three captcha keywords, so they always fell through to the `default` branch | Fixed — `KeywordEngine` now routes all three to `CaptchaSolver`; `locatorKey` = captcha image, `testData` = ObjectRepository key of the answer input field |
| `TessDataPath ... does not exist` (or a native tess4j crash) from any captcha keyword, only in Docker/CI | `CaptchaSolver`'s tessdata path was hardcoded to the Windows install path with no config wiring, and the Docker image never installed `tesseract-ocr` at all | Fixed — `CaptchaSolver` now resolves the datapath via `tesseract.datapath` config / `TESSDATA_PREFIX` / common per-OS install paths, and the Dockerfile installs `tesseract-ocr` + `tesseract-ocr-eng` |
| `ConfigException: Could not locate a tessdata directory`, only on Jenkins/GitHub Actions/GitLab CI (worked fine in Docker) | `tesseract-ocr` was only ever installed in the `Dockerfile` — the three CI pipelines run `mvn test` directly on the agent/runner, which never had it installed | Fixed — all three pipelines now install `tesseract-ocr`/`tesseract-ocr-eng` (install-if-missing, matching each pipeline's existing browser-install pattern) before running tests |
| `CaptchaSolver` existed and worked when a keyword script explicitly used `SOLVE_TEXT_CAPTCHA`, but a plain Page-Object test (or any page that never mentions a CAPTCHA keyword) would silently type garbage / fail past a CAPTCHA it never checked for | `CaptchaSolver` was only ever invoked from `KeywordEngine` — `BasePage`, `BaseMobilePage`, and every non-keyword Page Object had no path to it at all | Fixed — added static `CaptchaSolver.detectCaptchaImage/detectCaptchaInputField/isKnownUnsolvableCaptchaPresent()` plus `handleCaptchaIfPresent()` in both `BasePage` (called from `navigateTo()`) and `BaseMobilePage`, and an equivalent auto-check after every `NAVIGATE` step in `KeywordEngine` — see [🧩 CAPTCHA Solver](CAPTCHA_SOLVER.md) |
